...
import { trieSearch, trieDataset } from './trie';
...
export const AutocompleteDropdown = memo(
  forwardRef((props, ref) => {
    ...
    const source = useMemo(() => trieSearch);
    const sourceDataset = useMemo(() => trieDataset);
    const [dataSet, setDataSet] = useState(sourceDataset);
    ...
    const _onSelectItem = useCallback(item => {
      setSelectedItem(typeof item === 'object' ? {...item} : item)
    ...
    useEffect(() => {
      ...
      if (!Array.isArray(sourceDataset) || props.useFilter === false) {
        return
      }

      const findWhat = ignoreAccents ? diacriticless(searchText.toLowerCase()) : searchText.toLowerCase();

      const newSet = source.search(findWhat);

      setDataSet(newSet)
    }, [searchText, props.useFilter, ignoreAccents, matchFromStart]);
